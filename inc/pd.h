...
class PD {
public:
    PD(uint32_t sets = LLC_SET, uint32_t prof_ratio = 64, uint32_t prof_siz = 32, uint32_t reuse_cnt_stp = 4, uint32_t stge = 512 * 1024);
    ~PD();
...
private:
    ...
    uint32_t reuse_cnt_step;        // sets within continuous reuse_cnt_step will be cnted by the same reuse_dis_cnter
